...
def GAN_ISR_Batch_inf(gan_G, dataset, output_dir, batch_size, verbose=False):
    ...
    for idx in range(batch_size):   
        G_input, _, image_name = dataset[idx]
        ...
        resolved_image = gan_G(G_input)

        resolved_image = resolved_image.squeeze()

        # TO DO: EVALUATION e.g psnr etc.
...
if __name__ == '__main__':
    ...
    time_now = datetime.now().strftime("%Y_%m_%d_%p%I_%M")
